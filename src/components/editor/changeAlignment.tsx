...
type alignemntOpts = { fullName?: boolean } & (| ({ type?: 'button' } & Partial<ButtonProps>)
...
export const ChangeAlignment = ({ type = 'button', fullName = false, ...props }: alignemntOpts) => {
  ...
  const AlignmentItem = ({ icon = currentAlignmet }: { icon?: string }) => {
    return fullName ? <div className='capitalize'>align {icon} <AlignmentIcon icon={icon} /> </div> : <AlignmentIcon icon={icon} />
  }

  if (type === 'tabs') return (
    <Tabs aria-label='Change Alignment' variant='light' size='sm' {...props as TabsProps} selectedKey={currentAlignmet} onSelectionChange={(e) => setAlignment(e as string)}>
      {alignments.map(a => <Tab key={a}
        title={<AlignmentItem icon={a} />}
      />
      )}
    </Tabs>
  )
    ...
    <Popover {...(props as { popoverProps?: PopoverProps }).popoverProps}  >
      ...
      <PopoverContent>
        <Listbox variant='flat' selectedKeys={[currentAlignmet]} classNames={{ base: "m-0 p-0" }}>
          {alignments.map(a =>
            <ListboxItem onPress={() => setAlignment(a)}   > <AlignmentItem icon={a} /></ListboxItem>
          )}
        </Listbox>
      </PopoverContent>
    </Popover>
  ...
  if (type === 'button') return (
    <Button isIconOnly {...props as ButtonProps} onPress={() => {
      setAlignment(alignments[(alignments.findIndex((e) => e === currentAlignmet) + 1) % alignments.length])
    }}><AlignmentItem /> </Button>
  )
};
